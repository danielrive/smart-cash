locals {
  this_service_name     = "payment-orchestrator"
  this_service_port     = 8088
...
####### IAM policy for orchestrator service

resource "aws_iam_policy" "orchestrator_iam_policy" {
  name        = "policy-${local.this_service_name}-${var.environment}"
  path        = "/"
  description = "policy for payment orchestrator service account"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        # DynamoDB permissions - Update payment status
        Action = [
          "dynamodb:UpdateItem",
          "dynamodb:GetItem",
        ...
        Effect = "Allow"
        Resource = [
          data.aws_dynamodb_table.payment_table.arn
        ]
      },
      {
        # SQS permissions - Consume messages from payment-service-queue
        Action = [
          "sqs:ReceiveMessage",
          "sqs:DeleteMessage",
        ...
        Effect = "Allow"
        Resource = [
          data.aws_sqs_queue.payment_service_queue.arn
        ]
      }
    ]
  })
